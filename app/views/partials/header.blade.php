		...
		<nav id="main_menu">
			<ul class="primary_menu">
				...
				@if((Auth::check()) && (Auth::user()->access_level == 3) && (Auth::user()->roles->contains(3)))
					...
					@foreach((MenuItem::where('menu_id','=','1')->orderBy('sort_order')->remember(10)->get()) as $item)
					...
					@endforeach
				@else
					@foreach((MenuItem::where('menu_id','=','1')->where('active','=','1')->orderBy('sort_order')->remember(10)->get()) as $item)
						@if ($item->has_children == 0)
							@if ($item->page_id == 0)
