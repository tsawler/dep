    ...
    if (!isset($authenticated)) {
        ...
        if (!isset($bootstrap_file_found) || $bootstrap_file_found) {
            ...
            if (user_access('access kcfinder')) {
                if (!isset($_SESSION[COOKIE_PREFIX.'KCFINDER'])) {
                    $_SESSION[COOKIE_PREFIX.'KCFINDER'] = array();
                    $_SESSION[COOKIE_PREFIX.'KCFINDER']['disabled'] = false;
                }

                // User has permission, so make sure KCFinder is not disabled!
                if(!isset($_SESSION[COOKIE_PREFIX.'KCFINDER']['disabled'])) {
                    $_SESSION[COOKIE_PREFIX.'KCFINDER']['disabled'] = false;
                }

                global $user;
                $_SESSION[COOKIE_PREFIX.'KCFINDER']['uploadURL'] = strtr(variable_get('kcfinder_upload_url', 'sites/default/files/kcfinder'), array('%u' => $user->uid, '%n' => $user->name));
                $_SESSION[COOKIE_PREFIX.'KCFINDER']['uploadDir'] = strtr(variable_get('kcfinder_upload_dir', ''), array('%u' => $user->uid, '%n' => $user->name));
                $_SESSION[COOKIE_PREFIX.'KCFINDER']['theme'] = variable_get('kcfinder_theme', 'oxygen');

                //echo '<br />uploadURL: ' . $_SESSION[COOKIE_PREFIX.'KCFINDER']['uploadURL']<br />;
                //echo '<br />uploadDir: ' . $_SESSION[COOKIE_PREFIX.'KCFINDER']['uploadDir']<br />;
